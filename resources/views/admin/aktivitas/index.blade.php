...
<style>
    ...
    .tb-akt-search { position: relative; display: flex; align-items: center; flex: 1 1 220px; min-width: 0; }
    .tb-akt-search > svg {
    ...
    .tb-akt-toolbar .tb-select { flex: 1 1 160px; min-width: 0; max-width: 100%; height: 46px; }
    @media (max-width: 767.98px) {
        .tb-akt-toolbar .tb-btn,
        .tb-akt-toolbar .tb-select,
        .tb-akt-search { flex: 1 1 100%; width: 100%; }
    ...
    .tb-akt-badge svg { width: 0.8rem; height: 0.8rem; }

    /* Pengguna */
    .tb-akt-user { display: flex; align-items: center; gap: 0.5rem; }
    .tb-akt-avatar {
    ...
    .tb-akt-user-info { min-width: 0; }
    .tb-akt-user-name { font-size: 0.82rem; font-weight: 600; color: var(--tb-ink); line-height: 1.2; }
    .tb-akt-user-role { font-size: 0.7rem; color: var(--tb-muted); text-transform: capitalize; }
...
<div class="tb-section-head" style="margin-bottom:1rem;">
    <div class="tb-section-head-left">
        ...
        <div>
            <h2 class="tb-section-title">Log Aktivitas</h2>
            <p class="tb-section-desc">Riwayat semua aksi penting: master data, mahasiswa, preferensi, & permintaan belajar.</p>
        </div>
    </div>
</div>
...
{{-- ============ FILTER ============ --}}
@php
    $filterAktif = request('q') || request('kelompok') || request('user_id');
@endphp
<div class="tb-search-card">
    <form method="GET" action="{{ route('admin.aktivitas.index') }}" class="tb-akt-toolbar tb-instant-search">
        ...
        <select name="user_id" class="tb-select">
            <option value="">Semua Pengguna</option>
            @foreach ($userList as $id => $nama)
                <option value="{{ $id }}" @selected((string) request('user_id') === (string) $id)>{{ $nama }}</option>
            @endforeach
        </select>
        @if ($filterAktif)
            <a href="{{ route('admin.aktivitas.index') }}" class="tb-btn tb-btn-ghost">
        ...
        <div class="tb-search-active">
            ...
            @if (request('q'))
                <span class="tb-search-chip">
                    "{{ request('q') }}"
                    <a href="{{ route('admin.aktivitas.index', array_filter(request()->except('q'))) }}" title="Hapus pencarian"><x-icon name="x-lg" /></a>
                </span>
            @endif
            @if (request('kelompok'))
                <span class="tb-search-chip">
                    {{ $kelompokList[request('kelompok')] ?? request('kelompok') }}
                    <a href="{{ route('admin.aktivitas.index', array_filter(request()->except('kelompok'))) }}" title="Hapus filter kategori"><x-icon name="x-lg" /></a>
                </span>
            @endif
            @if (request('user_id') && isset($userList[request('user_id')]))
                <span class="tb-search-chip">
                    {{ $userList[request('user_id')] }}
                    <a href="{{ route('admin.aktivitas.index', array_filter(request()->except('user_id'))) }}" title="Hapus filter pengguna"><x-icon name="x-lg" /></a>
                </span>
            @endif
...
<div class="tb-card">
    ...
    @if ($aktivitas->isEmpty())
        <div class="tb-empty">
            ...
            <p class="tb-empty-desc">Riwayat aksi admin & mahasiswa akan muncul di sini.</p>
        </div>
    @else
                ...
                <tbody>
                    @foreach ($aktivitas as $log)
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($log->user)
                                    <div class="tb-akt-user">
                                        <div class="tb-akt-avatar">{{ strtoupper(substr($log->user->nama, 0, 1)) }}</div>
                                        <div class="tb-akt-user-info">
                                            <div class="tb-akt-user-name">{{ $log->user->nama }}</div>
                                            <div class="tb-akt-user-role">{{ $log->user->role }}</div>
                                        </div>
                                    </div>
                                @else
                                    <span class="tb-muted tb-text-sm">(akun terhapus)</span>
                                @endif
                            </td>
                            <td>
